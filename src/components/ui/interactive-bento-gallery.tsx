    ...
    return (
        <>
            ...
            >
                ...
                <div className="h-full flex flex-col">
                    <div className="flex-1 p-2 sm:p-3 md:p-4 flex items-center justify-center bg-gray-50/50">
                        <AnimatePresence mode="wait">
                            <motion.div
                                key={selectedItem.id}
                                className="relative w-full aspect-[16/9] max-w-[98%] sm:max-w-[90%] md:max-w-3xl 
                ...
                <motion.button
                    className="absolute top-2 sm:top-2.5 md:top-3 right-2 sm:right-2.5 md:right-3 
                              p-1.5 sm:p-2 rounded-full bg-gray-200/80 text-gray-700 hover:bg-gray-300/80 
            ...
            <motion.div
                ...
                className="fixed z-50 left-1/2 bottom-2 sm:bottom-4 -translate-x-1/2 touch-none"
            >
                <motion.div
                    ...
                    <div className="flex items-center -space-x-1 sm:-space-x-2 px-2 sm:px-3 py-1 sm:py-2">
                        {mediaItems.map((item, index) => (
                            <motion.div
                                ...
                                className={`
                                    relative group
                                    w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 flex-shrink-0 
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems, title, description }) => {
    ...
    return (
        <div className="container mx-auto px-1 sm:px-2 md:px-3 py-1 sm:py-2 md:py-3 max-w-full">
            <div className="mb-2 sm:mb-3 md:mb-4 text-center">
                <motion.h1
                    className="text-lg sm:text-xl md:text-2xl font-bold bg-clip-text text-transparent 
                             ...
                             dark:from-blue-300 dark:via-blue-500 dark:to-blue-300"
                ...
                <motion.p
                    className="mt-1 text-xs sm:text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
            ...
            <AnimatePresence mode="wait">
                {selectedItem ? (
                    ...
                    <motion.div
                        className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-1.5 sm:gap-2 md:gap-2.5 auto-rows-[35px] sm:auto-rows-[40px] md:auto-rows-[45px]"
                        initial="hidden"
                        animate="visible"
                        exit="hidden"
                        variants={{
                    ...
                    >
                        {items.map((item, index) => (
                            ...
                            >
                                ...
                                />
                                <motion.div
                                    className="absolute inset-0 flex flex-col justify-end p-1 sm:p-1.5 md:p-2"
                                    initial={{ opacity: 0 }}
                                    whileHover={{ opacity: 1 }}
                                    transition={{ duration: 0.2 }}
                                >
                                    <div className="absolute inset-0 flex flex-col justify-end p-1 sm:p-1.5 md:p-2">
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />
                                        <h3 className="relative text-white text-[8px] sm:text-[10px] md:text-xs font-medium line-clamp-1 leading-tight">
                                            {item.title}
                                        </h3>
                                        <p className="relative text-white/70 text-[6px] sm:text-[8px] md:text-[10px] line-clamp-1 leading-tight">
                                            {item.desc}
                                        </p>
                                    </div>
                                </motion.div>
